...
def test_AAA_BNS(tpath_join, tpath_preclear, pprint):
    ...
    def fitplot(
        ...
        axB.ax0.semilogy(F_Hz, abs(TF3), label = 'Bary. ZPK (order {})'.format(ZPKorder(results.zpk)))
        ...
        axB.ax0.set_title("AAA Fit, {} Points, max rel. error {:0.0e}, fit time {}".format(len(F_Hz), res_tol, timer))
        axB.ax0.legend(framealpha=1)
        return axB
...
def test_AAA_GWINC(tpath_join, tpath_preclear, pprint):
    ...
    ax = fig.add_subplot(1, 1, 1)
    ax.set_title("PyGWINC aLIGO noise budget\nAAA Fit, {} Points, max rel. error {:0.0e}, fit time {}".format(len(F_Hz), 1e-4, timer))
    ax.set_ylabel("Strain ASD [h/rtHz]")
